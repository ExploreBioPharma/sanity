...
const defaultUniqueCheck = (slug, options) => {
...
const SLUG_CORE = {
  ...
  validation: Rule =>
    Rule.custom((value, options) => {
      ...
      const isUnique = get(options, 'type.options.isUnique', defaultUniqueCheck)
      return Promise.resolve(isUnique(value.current, {...options, defaultUniqueCheck})).then(
